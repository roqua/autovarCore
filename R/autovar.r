...
#'
#' AutovarCore evaluates eight kinds of models: models with and without log transforming the data, lag 1 and lag 2 models, and with and without weekday dummy variables. For each of these 8 model configurations, we evaluate all possible combinations for including outlier dummies (at 2.5x the standard deviation of the residuals) and retain the best model (the procedure for selecting the best model is described in more detail below). All models considered include day-part dummy variables if there are multiple measurements per day. For the lag 2 models, all cross-lagged relations of the second lag are constrained.
#'
#' These eight models are further reduced to four models because AutovarCore determines whether adding weekday dummies improves the model fit (considering only significance bucket and the AIC/BIC score, NOT the number of dummy variables), and only when the best model found with weekday dummies is a "better model" than the best model found without weekday dummies (other parameters kept the same), do we include the model with weekday dummies and discard the one without weekday dummies. Otherwise, we include only the model without weekday dummies and discard the one with weekday dummies.
#'
#' Thus, AutovarCore always returns four models (assuming that we find enough models that pass the Eigenvalue stability test: models that do not pass this test are immediately discarded). There are three points in the code where we compare models to each other, which is done according to what we refer to as algorithm A or algorithm B, which we explain below.
#'
#' When evaluating all possible combinations of outlier dummies for otherwise identical model configurations, we use algorithm A to determine the best model. When comparing whether the best model found without weekday dummy columns is better than the best model found with weekday dummy columns, we use algorithm B. For sorting the two models with differing lag but both being either logtransformed or not, we again use algorithm A. Then at the end we merge the two lists of two models (with and without logtransform) to obtain the final list of four models. The sorting comparison here uses algorithm B.
#'
#' The reason for the different sorting algorithms is that in some cases we want to select the model with the fewest dummy variables (i.e., the model that retains most of the original data), while in other cases we know that a certain operation (such as adding weekday dummies or logtransforming the data set) will affect the amount of weekday dummies in the model and so a fair comparison would exclude this property.
#'
...
#'
#' The significance buckets are formed between each of the (decreasingly sorted) specified \code{significance_levels} in the parameters to the autovar function call. For example, if the \code{signifance_levels} are \code{c(0.05, 0.01, 0.005)}, then the significance buckets are \code{(0.05 <= x), (0.01 <= x < 0.05), (0.005 <= x < 0.01),} and \code{(x < 0.005)}. The metric used to place a model into a bucket is the minimum p-value of all three residual tests of all endogenous variables in the model.
#' \item If the two models being compared are in the same significance bucket, the number of dummy variables is most important. If the two models being compared differ in the number of dummy variables, choose the one with the fewest dummy variables, otherwise proceed to step 3.
#'
#' For this count of dummy variables, the following rules apply: \itemize{
...
#' \item The inclusion of weekday dummies in a model adds one total to dummy variable count. This is because we do not necessarily want to punish models if the data happens to exhibit weekly cyclicity, but models that do not need the weekday dummies and are equally likely should be preferred.
#'
#' \item Each outlier dummy variable correponds to one outlier and adds one to the count of dummy variables. The outliers are, for each variable, the measurements at >2.5 times the standard deviation away from the mean of the residuals or of the squared residuals. Checks are in place to ensure that a time point identified as an outlier by multiple variables only adds a single outlier dummy variable to the equation.
#' }
#' \item When the bucket and number of dummy variables for the two models being compared are the same, select the one with the lowest AIC/BIC score. Whether the AIC or BIC is used here depends on the  \code{criterion} option specified in the parameters to the autovar function call.
#' }
#'
#'
#' In the end, we should have one best logtransformed model and one best nonlogtransformed model. We then compare these two models in the same way as we have compared all other models up to this point with one exception: we do not compare the number of dummy variables. Comparing the number of dummy variables would have likely favored logtransformed models over models without logtransform, as logtransformations typically have the effect of reducing the outliers of a sample.
#'
#' Algorithm B is identical to algorithm A, except that we skip the step comparing the number of dummy variables. Thus, we instead compare by bucket first and AIC/BIC score second. Notice that, if we may assume that the presence or absence of weekday dummies does not vary between the four models for any particular invocation of the autovar method (which is not an unreasonable assumption to make), that then the arbitrary choice of letting all weekday dummy columns together add one to the outlier count does not matter at all, since the only times where we are comparing the number of dummy variables is when both models either both have or both do not have weekday dummy columns.
#'
...
#' @param imputation_iterations The amount of times the Amelia imputation should be averaged over. The default value for this parameter is \code{100}. For details on the Amelia imputation, please see \url{http://r.iq.harvard.edu/docs/amelia/amelia.pdf}.
#' @param measurements_per_day The number of measurements per day in the time series data. The default value for this parameter is \code{1}. If this value is \code{0}, then daypart- and weekday dummies variables are not included for any models.
...
autovar <- function(raw_dataframe, selected_column_names, significance_levels = c(0.05, 0.01, 0.005),
                    ...
                    criterion = 'AIC', imputation_iterations = 100, measurements_per_day = 1) {
